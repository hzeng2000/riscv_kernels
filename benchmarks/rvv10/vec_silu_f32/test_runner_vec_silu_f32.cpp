...
// Auto-generated test runner for ggml_vec_silu_f32 on sg2044
// RVV Version: rvv_1.0
...
std::string short_name(const std::string& full_name) {
    const std::string prefix = "ggml_vec_silu_f32_";
...
void generate_random_floats(std::vector<float>& vec) {
    ...
    std::uniform_real_distribution<> dis(-3.0f, 3.0f);
...
int main(int argc, char** argv) {
    ...
    std::cout << "=== ggml_vec_silu_f32 Benchmark on sg2044 ===" << std::endl;
    ...
    ggml_vec_silu_f32_scalar(config.n, y_baseline.data(), x_f32.data());
    
    const float tolerance = 1e-4f;
    bool all_correct = true;
    for (auto const& test : all_tests) {
        test.func(config.n, y_result.data(), x_f32.data());
