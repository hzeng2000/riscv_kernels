...
BASE_DIR="${SCRIPT_DIR}/out/phase_unzfh"
...
# 日志文件名称，包含时间戳
LOG_FILE="${BASE_DIR}/benchmark_phase_unzfh_$(date +%Y%m%d_%H%M%S).log"
...
for variant in "${VARIANTS[@]}"; do
    ...
    # 使用 find 命令递归查找
    find "$MODEL_ROOT_DIR" -type f -name "*.gguf" | sort | while read model_path; do
